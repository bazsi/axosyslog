...
A new `http()` source (in the `http-source` module, built on a vendored llhttp parser
driven by the ivykis event loop, with no external HTTP-server dependency) accepts
